...
-- Migração segura: adicionar campos opcionais se não existirem
DO $$
BEGIN
  BEGIN
    ALTER TABLE partida ADD COLUMN IF NOT EXISTS local TEXT;
  EXCEPTION WHEN others THEN NULL; END;
  BEGIN
    ALTER TABLE partida ADD COLUMN IF NOT EXISTS transmissao TEXT;
  EXCEPTION WHEN others THEN NULL; END;
  BEGIN
    ALTER TABLE partida ADD COLUMN IF NOT EXISTS placar VARCHAR(15);
  EXCEPTION WHEN others THEN NULL; END;
  BEGIN
    ALTER TABLE partida ADD COLUMN IF NOT EXISTS finalizada BOOLEAN DEFAULT FALSE;
  EXCEPTION WHEN others THEN NULL; END;
END$$;
